...
const SDRecordPage = () => {
    ...
    useEffect(() => {
        const fetchInstrumentsBySdg = async () => {
            try {
                // Fetch instruments based on the selected SDG
                setAnswers([]);
                const response = await fetch(
                                ...
                                await Promise.all(
                                    questions.map(async (question) => {
                                        const values = await fetch(
                                            "http://localhost:9000/api/records-values",
                                            {
                                                ...
                                                body: JSON.stringify({
                                                    ...
                                                    selectedYear: selectedYear, // Add selectedYear
                                                    selectedSdg: selectedSdg, // Add selectedSdg
                                                }),
                                            }
                                        );
                ...
                // Update state with fetched instruments and answers
    ...
    }, [selectedSdg, selectedYear]); // Dependency array includes both selectedSdg and selectedYear
    ...
    }, [answers, instruments, selectedSdg, selectedYear]);
    ...
    //     // outer.post("/records-values/2", async (req, res) => {
    //     //     try {
    ...
    const handleSubmit = async (e) => {
        ...
        // Check if record exists for the current user, SDG, and year
        const checkResponse = await fetch(
            "http://localhost:9000/api/records-values/check",
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
        ...
        if (checkResponse.ok) {
            const { record_id } = await checkResponse.json();

            // Record exists, update answers
            const updateAnswers = async (answers) => {
                const url = "http://localhost:9000/api/update/answers"; // Create a separate update route

                for (const answer of answers) {
                    const data = {
                        record_value_id: answer.questionId,
                        value: answer.answer,
                        question_id: answer.questionId,
                        record_id: record_id, // Use the existing record_id
                    };
            ...
            // No record exists, create a new one and add answers
            const response = await fetch(
            ...
            console.error("Failed to check record existence.");
        }
    };
