...
const ScorePerCampusChart = () => {
    ...
    const handleSelectSdg = (sdg_id) => {
        ...
        setColor(selectedSdgColor || "#E5243B"); // Default color if not found
    };
        ...
        <Card className="w-[75%]">
            ...
            <BarChart
                data={campuses.map((campus) => ({
                    //name remove teht word Campus and BatStateU -
                    name: campus.name
                        .replace("Campus", "")
                        .replace("Pablo Borbon", "Main")
                        .replace("BatStateU - ", ""),
                    score: campus.score.toFixed(2),
                }))}
                index="name"
                categories={["score"]}
                colors={["blue"]} // Pass color as an array
                yAxisWidth={50}
                className="mt-6 h-60"
            />
        </Card>
